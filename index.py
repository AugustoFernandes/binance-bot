...
#settings, всегда оставляйте 11 строку пустой
...
#Bot class
class binance():
    # Все доступные методы из документации 
    methods = {
            # public methods
    ...
    def call_api(self, **kwargs):
        ...
        print("\n[!] REQUESTED URL >> ", api_url, payload_str, self.methods[command])
...
### Web Socket API - получение инфы по валютной паре
# def getSocketStream():
...
#     def on_open(ws):
#         print("[!] CONNECTED [!]")
#     # Raw mode "wss://stream.binance.com:9443/ws/%s@aggTrade/%s@aggTrade"
#     # Combined mode "wss://stream.binance.com:9443/stream?streams=%s@aggTrade/%s@aggTrade"
#     # if __name__=="__main__":
#     ws = websocket.WebSocketApp("wss://stream.binance.com:9443/stream?streams=%s@aggTrade/%s@aggTrade"%(smbl.lower(), smbl.lower()),on_message=on_message,on_error=on_error,on_close=on_close)
#     ws.on_open = on_open
#     ws.run_forever()
# getSocketStream()

# Инициализируем бота и вызываем методы, ключи подтягиваем из настроек
bot = binance(API_KEY = API_KEY, API_SECRET = API_SECRET)

# # Метод "создать тестовый ордер", подробнее про create order см. в Readme.md
# print("[*] TEST ORDER for %s"%smbl, bot.testOrder(
...
#     timeInForce = 'GTC',  # GTC (Good Tindex.write(setEntr)ill Cancelled)  - ордер активен пока его не отменят
#     quantity = 1,         # Количество покупки
#     price = 5600,         # Цена
# ))
# # Метод "отменить ордер", сюда нужно всегда передавать orderid(int, not string) и валютную пару  
# # print("[*] CANCEL ORDER", bot.cancelOrder(
# #     orderid = ,
# #     symbol = smbl,
# # ))

# # Метод "получить список созданных(открытых) ордеров"
# print("[*] ORDERS LIST for %s"%smbl, bot.openOrders(
#     symbol = smbl,
# ))
# # Если нужен список ордеров по всем парам  - закомментите предыдущий метод и раскоментите этот:
# # print("[*] ALL ORDERS LIST", bot.openOrders())

# # Метод "История сделок пользователя"
# print("[*] TRADES HISTORY for %s"%smbl, bot.myTrades(
#     symbol = smbl,
# ))

# # Метод "Информация по аккаунту" или баланс, ответ как обычно в жейсоне в массиве "balances: [...]"
# print("[*] ACCOUNT INFORMATION", bot.account())
